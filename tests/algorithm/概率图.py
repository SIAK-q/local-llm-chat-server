...
class Factor:
    ...
    def diff(self, other):
        if (self.query_ob_value != None):
            return (np.sum(
                [abs(self.get(un_value, self.query_ob_value) - other.get(un_value, self.query_ob_value)) for un_value in
                 Factor.enumValues(self.un_mask)]))
    ...
    def sum(self, index_list):
        ...
        for un_value in Factor.enumValues(other_mask):
            sum_factor.factor_table[0][un_value] = np.sum(
                [flattern_factor.evaluate(un_value | sum_over_value) for sum_over_value in
                 Factor.enumValues(sum_over_mask)], axis=0)
        # print("\nSum:")
    ...
    def query(self, query_list, observe_map, method="BruteForce", true_dist_filename=None, ve_order=[]):
        ...
        for un_value in Factor.enumValues(result.un_mask):
            if (method == "BruteForce"):
                numerator = np.sum(
                    [self.jointDist(ot_value | un_value | ob_value) for ot_value in Factor.enumValues(ot_mask)])
                denominator = np.sum(
                    [self.jointDist(ot_un_value | ob_value) for ot_un_value in Factor.enumValues(ot_un_mask)])
                prob = numerator / denominator
            elif (method == "TrueDistribution"):
                numerator = np.sum(
                    [true_dist_table[ot_value | un_value | ob_value] for ot_value in Factor.enumValues(ot_mask)])
                denominator = np.sum(
                    [true_dist_table[ot_un_value | ob_value] for ot_un_value in Factor.enumValues(ot_un_mask)])
                prob = numerator / denominator
        ...
        query_list, observe_map, method, runtime, result))
        return (result)
